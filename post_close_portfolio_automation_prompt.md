...
4. 比较实际组合、原样持有基准和标准化建议反事实。建议反事实以当时留档的 `recommended_exposure` 为准，但收益起点必须是 `EMAIL_SENT` 后第一根精确、完整的一分钟行情，不能使用报告生成时更早的分析参考价；缺失时保持 `PENDING_DATA`。统一假设在可执行参考价即时调整并扣除变更 exposure 的成本；这只是可比基准，不得写成条件已真实触发。成交时间仅为通知代理或手续费缺失时标为 `MATURED_PARTIAL`。
