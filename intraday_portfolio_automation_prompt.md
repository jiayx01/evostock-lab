...
5. 生成本次唯一 `run_id`，执行 `.venv/bin/python automation_lock.py acquire --name intraday --run-id <run_id> --stale-minutes 75`。返回 `BUSY` 时说明上一轮仍在运行，本轮记录跳过且不读信、不分析、不发信。获得锁后先检查 `data/decision_log.jsonl` 中该 scheduled_slot：已有 `EMAIL_SENT` 时直接幂等跳过；只有未完成发送状态才进入恢复流程。无论成功、失败或门禁停止，退出前都必须用同一 run_id 执行 `release`。
...
```bash
.venv/bin/python commit_broker_sync_batch.py --input <本轮已核验批次JSON>
```
