...
*图 1｜当次可见证据经过完整性门禁形成决策 episode；只有按固定窗口结算、通过时间顺序验证并经用户确认的规则，才能反馈到下一次生产判断。*
...
![图 2：EvoStock Lab 持久化记忆的分层与权限边界](assets/evostock-memory-architecture.svg)

*图 2｜实线表示可影响生产判断的受控数据流；虚线表示可丢弃的运行检查点或不具规则权限的候选经验。私有数据通过加密快照和 manifest 恢复，不进入 Git。*
